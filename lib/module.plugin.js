import { log, getTrackingUserId, optInUser, helloRetail } from './ralph-module-hello-retail.utils'
...
export default function(app, inject) {
  ...
  // opting in user to tracking if cookie available and setting the Hello Retail tracking User ID
  optInUser(() => {
    getTrackingUserId();
  });
    ...
    if (mutation.type === 'events/push') {
      ...
      switch (eventType) {
        ...
        case 'user:login':
          ...
          // // helloRetail('setCustomerEmail', eventData.__ob__?.dep.subs[0]?.value?.auth?.user);
          break;
        ...
        case 'newsletter:subscribe':
          ...
          helloRetail('setCustomerEmail', eventData.email);
          break;
