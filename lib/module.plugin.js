import { log, trackEvent, beginTracking } from './ralph-module-hello-retail.utils'
...
export default function(app, inject) {
  ...
  // if cookie exists set opt out to false and set the Hello Retail tracking User ID
  beginTracking();
  ...
  app.store.subscribe((mutation, state) => {
    if (mutation.type === 'events/push') {
      ...
      switch (eventType) {
        ...
        case 'cart:remove':
          const cartObj = {
            urls: state.cart.data.items.map(item => item.product.canonicalUrl),
            productNumbers: state.cart.data.items.map(item => item.product.productId)
          }
          trackEvent('setCart', cartObj);
          break;
        ...
        case 'product-detail:impression':
          ...
          trackEvent(
            'trackPageView', 
            eventData.product.canonicalUrl, 
